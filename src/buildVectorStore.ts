...
export interface BuildVectorStoreOptions {
  persistDir?: string;
  /**
   * Ollama baseUrl，默认 http://127.0.0.1:11434
   */
...
export function createEmbeddings(options: BuildVectorStoreOptions = {}) {
...
export function loadVectorStore(
  options: BuildVectorStoreOptions = {}
): Promise<HNSWLib> {
 ...
 */
export function buildOrLoadVectorStore(
  ...
  if (!documents || documents.length === 0) {
    return Promise.reject(
      new Error(
        `向量库不存在（${persistDir}），且未提供可用于构建的 documents。请先加载并切分 .docs 文档。`
      )
    );
  }
  ...
  return HNSWLib.fromDocuments(documents, embeddings).then((vs) =>
    vs.save(persistDir).then(() => vs)
  );
}

export function deleteVectorStore(options: BuildVectorStoreOptions = {}) {
  const persistDir = options.persistDir ?? DEFAULT_PERSIST_DIR;
  if (fs.existsSync(persistDir)) {
